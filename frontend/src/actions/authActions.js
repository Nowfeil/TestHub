    ...
    .then((data) => {
      if (data.token) {
        ...
        dispatch({
          ...
          profileID: data.payload.profileID,
        });
      } else {
        dispatch({
          type: LOGIN_FAILURE,
  ...
  fetch("http://localhost:5000/user/signup", {
    ...
    .then((data) => {
      ...
      if (data.msg === "User Already Exists") {
        dispatch({ type: SIGN_UP_FAILURE, errMess: data.msg });
      } else if (data.token) {
        dispatch({ type: SIGN_UP_SUCCESS, user: data.user });
      }
    })
    .catch(() => {
      dispatch({ type: SIGN_UP_FAILURE, errMess: "Something went wrong!" });
    });
};
